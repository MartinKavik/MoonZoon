...
pub fn derive_local_serialize(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
...
fn expand_for_struct(
    ...
    let serialize = match data.fields {
        ...
        Fields::Unit => quote! {
            Ok(local_serde::Intermediate::None)
        },
    };

    quote! {
        #[allow(unused_variables)]
        impl #impl_generics local_serde::LocalSerialize<LocalSerdeType> for #name #ty_generics #where_clause {
            fn local_serialize(&self) -> Result<local_serde::Intermediate, local_serde::Error> {
                #serialize
            }
        }
    }
...
fn expand_struct_named_fields(fields: &FieldsNamed) -> TokenStream {
    ...
    quote! {
        ...
        Ok(local_serde::Intermediate::Map(__map))
    }
}
...
fn expand_struct_unnamed_fields(fields: &FieldsUnnamed) -> TokenStream {
    ...
    quote! {
        ...
        if __arr.is_empty() {
            ...
            Ok(local_serde::Intermediate::Array(__arr))
        }
    }
}
...
fn expand_internally_tagged_enum(tag: &str) -> TokenStream {
    ...
    quote! {
        if __content.is_none() {
            let mut __map = local_serde::Map::with_capacity(1);

            __map.insert(String::from(#ltag), local_serde::Intermediate::String(__tag));

            return Ok(local_serde::Intermediate::Map(__map));
        } else if let local_serde::Intermediate::Map(__map) = __content {
            let mut __res = local_serde::Map::with_capacity(__map.len() + 1);

            __res.insert(String::from(#ltag), local_serde::Intermediate::String(__tag));
            __res.extend(__map);

            return Ok(local_serde::Intermediate::Map(__res));
        } else if let local_serde::Intermediate::Array(__arr) = __content {
            if __arr.is_empty() {
                let mut __map = local_serde::Map::with_capacity(1);
        ...
        Err(local_serde::Error::custom("enum cannot be tagged internally"))
    }
}
...
fn expand_adjacently_tagged_enum(tag: &str, content: &str) -> TokenStream {
    ...
    quote! {
        let mut __map = local_serde::Map::with_capacity(2);
...
fn expand_externally_tagged_enum() -> TokenStream {
    quote! {
        if __content.is_none() {
            return Ok(local_serde::Intermediate::String(__tag));
        } else if let local_serde::Intermediate::Map(__map) = &__content {
            if __map.is_empty() {
                return Ok(local_serde::Intermediate::String(__tag));
            }
...
fn serialize_struct_enum_variant(variant: &Variant, fields: &FieldsNamed) -> TokenStream {
    ...
    quote! {
        Self::#ident { #deconstructor } => {
            ...
            let __content = local_serde::Intermediate::Map(__map);
...
fn serialize_tuple_enum_variant(variant: &Variant, fields: &FieldsUnnamed) -> TokenStream {
    ...
    quote! {
        Self::#ident ( #deconstructor ) => {
            ...
            if __arr.is_empty() {
                (__tag, local_serde::Intermediate::None)
            } else if __arr.len() == 1 {
                (__tag, __arr.pop().unwrap())
            } else {
                (__tag, local_serde::Intermediate::Array(__arr))
            }
        }
    }
}
...
fn serialize_unit_enum_variant(variant: &Variant) -> TokenStream {
    ...
    quote! {
        Self::#ident => {
            let __tag = String::from(#lname);
            let __content = local_serde::Intermediate::None;
...
fn serialize_named_fields(fields: &FieldsNamed) -> (TokenStream, TokenStream) {
    ...
    let mut serialize = quote! {
        let mut __map = local_serde::Map::with_capacity(#len);
    ...
    for field in &fields.named {
        ...
        let serialize_field = if skip {
            ...
            quote! {
                match #name.local_serialize() {
                    Ok(local_serde::Intermediate::Map(inner)) => __map.extend(inner),
                    Ok(_) => return Err(local_serde::Error::custom(
                        concat!("field \"", stringify!(#name), "\" cannot be flattened")
                    )),
                    Err(local_serde::Error::NamedFieldErrors(errors)) => {
    ...
    serialize.extend(quote! {
        if !__field_errors.is_empty() {
            return Err(local_serde::Error::NamedFieldErrors(__field_errors));
        }
    });
...
fn serialize_unnamed_fields(fields: &FieldsUnnamed) -> (TokenStream, TokenStream) {
    ...
    match fields.unnamed.len() {
        ...
        _ => serialize.extend(quote! {
            if !__field_errors.is_empty() {
                return Err(local_serde::Error::UnnamedFieldErrors(__field_errors));
            }
        }),
    }
