...
pub fn derive_local_deserialize(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
...
fn expand_for_struct(
    ...
    let expanded = quote! {
        ...
        impl #impl_generics local_serde::LocalDeserialize<LocalSerdeType> for #name #ty_generics #where_clause {
            fn local_deserialize(__val: &local_serde::Intermediate) -> Result<Self, local_serde::Error> {
                #deserialize
            }
        }
    };
...
fn expand_internally_tagged_enum(
    ...
    quote! {
        let __obj = __val.as_map().ok_or_else(|| local_serde::Error::invalid_value("object"))?;

        let __variant = __obj
            .get(#ltag)
            .map(|v| v.as_str())
            .ok_or_else(|| local_serde::Error::MissingField)
            .and_then(|v| v.ok_or_else(|| local_serde::Error::invalid_value("enum variant name")))
            .map_err(|err| local_serde::Error::NamedFieldErrors(vec![
        ...
        match __variant {
            #deserialize
            _ => Err(local_serde::Error::UnknownEnumVariant),
        }
    }
}
    ...
    quote! {
        if let Some(__obj) = __val.as_map() {
            #with_content
                ...
                Err(local_serde::Error::UnknownEnumVariant)
            }
            ...
            Err(local_serde::Error::invalid_value("enum variant"))
        }
    }
}
...
fn construct_enum_variant_without_content(
    ...
    if let Some(content) = content_field {
        ...
        quote! {
            Err(local_serde::Error::NamedFieldErrors(vec![
                (String::from(#lcontent), local_serde::Error::MissingField)
            ]))
        }
    } else {
        quote! {
            Err(local_serde::Error::MissingEnumVariantContent)
        }
    }
}
...
fn deserialize_named_fields(fields: &FieldsNamed) -> (TokenStream, TokenStream) {
    ...
    if !fields.named.is_empty() {
        deserialize.extend(quote! {
            let __obj = __val
                .as_map()
                .ok_or_else(|| local_serde::Error::invalid_value("object"))?;
    ...
    for field in &fields.named {
        ...
        if skip {
            ...
            deserialize.extend(quote! {
                let #name = match <#ty as local_serde::LocalDeserialize<LocalSerdeType>>::local_deserialize(__val) {
                    Ok(v) => Some(v),
            ...
            deserialize.extend(quote! {
                let #name = __obj
                    .get(#lname)
                    .map(<#ty as local_serde::LocalDeserialize<LocalSerdeType>>::local_deserialize)
            ...
            deserialize.extend(quote! {
                let #name = __obj
                    .get(#lname)
                    .map(<#ty as local_serde::LocalDeserialize<LocalSerdeType>>::local_deserialize)
                    .unwrap_or_else(|| Err(local_serde::Error::MissingField))
                    .map_err(|err| __field_errors.push((String::from(#lname), err)))
                    .ok();
            });
        }
...
fn deserialize_unnamed_fields(fields: &FieldsUnnamed) -> (TokenStream, TokenStream) {
    ...
    if !fields.unnamed.is_empty() {
        match fields.unnamed.len() {
            ...
            _ => deserialize.extend(quote! {
                let __arr = __val
                    .as_array()
                    .ok_or_else(|| local_serde::Error::invalid_value("array"))?;
            }),
        }
        ...
        deserialize.extend(quote! {
            if __arr.len() < #len {
                return Err(local_serde::Error::invalid_value(concat!("array of length ", #len)));
            }
    ...
    for (index, field) in fields.unnamed.iter().enumerate() {
        ...
        deserialize.extend(quote! {
            let #name = <#ty as local_serde::LocalDeserialize<LocalSerdeType>>::local_deserialize(&__arr[#lindex])
